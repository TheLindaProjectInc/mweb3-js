...
const { getMetrixRPCAddress, getDefaultMetrixAddress, getWalletPassphrase, isWalletEncrypted } = require('../../test/utils');

const { assert } = chai;

console.log(`Your Metrix RPC address is ${getMetrixRPCAddress()}`);
console.log(`Your Default Metrix address is ${getDefaultMetrixAddress()}`);

describe('Mweb3', () => {
  const METRIX_ADDRESS = getDefaultMetrixAddress();
  let mweb3;

  beforeEach(() => {
    mweb3 = new Mweb3(getMetrixRPCAddress());
  });

  /** ******** MISC ********* */
  describe('isConnected()', () => {
    it('returns true when connected', async () => {
      assert.isTrue(await mweb3.isConnected());
    });
  });
  ...
  describe('getBlockchainInfo()', () => {
    it('returns the block info', async () => {
      const res = await mweb3.getBlockchainInfo();
      assert.isDefined(res);
      assert.isObject(res);
  ...
  describe('getBlockCount()', () => {
    it('returns the blockcount', async () => {
      const res = await mweb3.getBlockCount();
      assert.isDefined(res);
      assert.isNumber(res);
    });
  });
  ...
  describe('getBlockHash()', () => {
    it('returns the block hash', async () => {
      const res = await mweb3.getBlockHash(0);
      assert.isDefined(res);
      assert.isString(res);
    });
  });
  ...
  describe('listContracts()', () => {
    it('returns the array of deployed contracts', async () => {
      const res = await mweb3.listContracts();
      assert.isDefined(res);
      assert.isObject(res);
    });
  });
  ...
  describe('searchLogs()', () => {
    ...
    it('throws if fromBlock is not a number', async () => {
      assert.throws(() => mweb3.searchLogs(
        'a', 50100, [],
  ...
  describe('getPeerInfo()', () => {
    it('returns the Node info', async () => {
      const res = await mweb3.getPeerInfo();
      assert.isDefined(res);
      assert.isArray(res);
  ...
  describe('getHexAddress()', () => {
    it('returns the hex address', async () => {
      const hexDecodedAddress = bs58.decode(MRX_ADDRESS).toString('hex');
      const hexadecimalAddress = await mweb3.getHexAddress(MRX_ADDRESS);
  ...
  describe('fromHexAddress()', () => {
    it('returns the Metrix address', async () => {
      const MetrixAddress = await mweb3.fromHexAddress('17e7888aa7412a735f336d2f6d784caefabb6fa3');
      assert.isString(MetrixAddress);
      assert.lengthOf(MetrixAddress, 34);
    });
  });
  ...
  describe('validateAddress()', () => {
    it('returns an object validating the address', async () => {
      let res = await mweb3.validateAddress(MRX_ADDRESS);
      assert.isDefined(res);
      assert.isDefined(res.isvalid);

      res = await mweb3.validateAddress('helloworld');
      assert.isDefined(res);
      assert.isDefined(res.isvalid);
    });
  });
  ...
  describe('dumpPrivateKey()', () => {
    it('returns the private key', async () => {
      const address = await mweb3.getAccountAddress('');
      if (await isWalletEncrypted(mweb3)) {
        await mweb3.walletPassphrase(getWalletPassphrase(), 3600);
        assert.isTrue((await mweb3.getWalletInfo()).unlocked_until > 0);

        const res = await mweb3.dumpPrivateKey(address);
        assert.isDefined(res);
        assert.isString(res);

        await mweb3.walletLock();
        assert.isTrue((await mweb3.getWalletInfo()).unlocked_until === 0);
      } else {
        const res = await mweb3.dumpPrivateKey(address);
        assert.isDefined(res);
  ...
  describe('getAccountAddress()', () => {
    it('returns the account address', async () => {
      const res = await mweb3.getAccountAddress('');
      assert.isDefined(res);
      assert.isString(res);
  ...
  describe('getAddressesByAccount()', () => {
    it('returns the account address array', async () => {
      const res = await mweb3.getAddressesByAccount('');
      assert.isDefined(res);
      assert.isArray(res);
  ...
  describe('getUnconfirmedBalance()', () => {
    it('returns the unconfirmed balance', async () => {
      const res = await mweb3.getUnconfirmedBalance();
      assert.isDefined(res);
      assert.isNumber(res);
    });
  });
  ...
  describe('listAddressGroupings()', () => {
    it('returns an array of address groupings', async () => {
      const res = await mweb3.listAddressGroupings();
      assert.isDefined(res);
      assert.isArray(res);
      ...
      if (!_.isEmpty(res)) {
        ...
        if (!_.isEmpty(innerArr)) {
          ...
          qAssert.isMetrixAddress(item[0]);
  ...
  describe('listLockUnspent()', () => {
    it('returns an array of unspendable outputs', async () => {
      const res = await mweb3.listLockUnspent();
      assert.isDefined(res);
      assert.isArray(res);
    });
  });
  ...
  describe('listUnspent()', () => {
    it('returns an unspent output array', async () => {
      const res = await mweb3.listUnspent();
      assert.isDefined(res);
      assert.isArray(res);
    });
  });
  ...
  describe('walletLock()', () => {
    it('locks the encrypted wallet', async () => {
      if (await isWalletEncrypted(mweb3)) {
        await mweb3.walletPassphrase(getWalletPassphrase(), 3600, true);
        assert.isTrue((await mweb3.getWalletInfo()).unlocked_until > 0);
  ...
  !_.includes(process.argv, '--cleanenv') ? describe.skip : describe('cleanEnv tests', () => {
    describe('getNewAddress()', () => {
      it('returns a new Metrix address', async () => {
        const res = await mweb3.getNewAddress('');
        assert.isDefined(res);
        assert.isString(res);
        assert.isTrue(res.startsWith('q') || res.startsWith('Q'));
      });
    ...
    describe('backupWallet()', () => {
      it('backup the wallet', async () => {
        const res = await mweb3.backupWallet(path.join(__dirname, '../../test/data/backup.dat'));
    ...
    describe('importWallet()', () => {
      it('throw an error if importing a non-existent file', async () => {
        try {
          await mweb3.importWallet(path.join(__dirname, '../../test/data/backup.dat'));
        } catch (err) {
      ...
      it('import the wallet from a wallet dump file', async () => {
        const res = await mweb3.importWallet(path.join(__dirname, '../../test/data/backup.dat'));
        assert.notTypeOf(res, 'Error');
      });
    });
  });
